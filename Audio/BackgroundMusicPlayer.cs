...
public class BackgroundMusicPlayer : MonoBehaviour
{
    ...
    public void FadeMusicToDefault()
    {
        FadeSoundTo(m_MusicAudioSource,.15f);
    }

    public void FadeMusicToLow()
    {
        FadeSoundTo(m_MusicAudioSource,.03f);
    }

    public void FadeToTimeStop()
    {
        FadeSoundTo(m_MusicAudioSource,.03f);
        PlayJustAmbient();
        FadeSoundTo(m_AmbientAudioSource,.15f);
    }

    public void FadeToTimeFlow()
    {
        FadeSoundTo(m_AmbientAudioSource,.03f);
        StopJustAmbient();
        FadeMusicToDefault();
    }

    public void FadeSoundTo(AudioSource audioSource,float finalVolume = .2f, float fadeTime = .2f)
    {
        StartCoroutine(VolumeFade(audioSource, finalVolume, fadeTime));
    }
